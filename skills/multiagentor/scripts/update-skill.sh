...
test -f "$version_file" || { echo "Missing skill VERSION: $version_file" >&2; exit 1; }
...
emit_result() {
  printf '{"current_version":"%s","latest_version":"%s","updated":%s,"method":"%s","backup_path":"%s","restart_required":%s}\n' \
    "$1" "$2" "$3" "$4" "$5" "$3"
}
...
if version_ge "$current" "$latest"; then
  emit_result "$current" "$latest" false none ""
  exit 0
fi
if [ "$check_only" = true ]; then
  emit_result "$current" "$latest" false available ""
  exit 0
fi
...
if command -v git >/dev/null 2>&1 && worktree=$(git -C "$skill_root" rev-parse --show-toplevel 2>/dev/null); then
  ...
  case "$origin" in
    ...
    *) echo "A newer official skill exists, but this Git worktree origin is not the official repository: $origin" >&2; exit 1 ;;
  esac
  test -z "$(git -C "$worktree" status --porcelain)" || {
    echo "A newer skill exists, but the Git worktree has local changes: $worktree" >&2
    exit 1
  }
  ...
  emit_result "$current" "$installed" true git-ff-only ""
  exit 0
fi
...
emit_result "$current" "$archive_version" true github-archive "$backup"
